...
export class DocumentoService {
  ...
  async processDocument(messageReceived, senderNumber) {
    ...
    if (!conversationOpened.vectorStoreId) {
      ...
      const updateConversation =
        await this.conversationService.updateVectorStoreInBd(
    ...
    const conversationOpenedUpdated =
      await this.conversationService.findOpenedConversation(user.id);
    const fileVectorStore = await this.addFileToVectorStore(
      fileId,
      conversationOpenedUpdated.vectorStoreId,
    );
  ...
  async createThreadAndGetMessage(message: string, sender: string) {
    ...
    const messageInUserDb =
      await this.conversationService.createMessage(messageUser);
    ...
    const conversation = await this.conversationService.getConversationMessage(
      conversationOpened.id,
    );
    ...
    const runId = await this.runThread(
      threadId,
    ...
    respostaGpt = await this.conversationService.getMessages(threadId);
  ...
  async createThreadWithFile(input: string, vectorStoreId: string) {
    const response = await axios.post(
