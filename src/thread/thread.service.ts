...
export class ThreadService {
    constructor(private replyService: ReplyService, private conversationService: ConversationService, private menuService: MenuService, private userService: UserService, private documentService: DocumentoService, private eventService: EventService) { }
        ...
        if (message.text) {
            ...
            if (detectMenu(text)) {
                ...
                if (openedConversation) {
                    const ccvs = await this.conversationService.desactiveLastConversation(openedConversation.id);
                }
            ...
            if (conversationOpened.threadId) {
                ...
                const createNewMessageOpenAI = await this.conversationService.createNewMessageOpenAI(
            ...
            const { transcript, sender } = await processAudio(
                message,
                senderNumber,
            );
            const user = await this.userService.findUser(senderNumber);
    ...
    async leadConversation(message, senderNumber: string) {
        ...
        if (message.text) {
            ...
            if (text == 'Oi, EstagIArio! Quero entrar para a lista de espera.') {
                await this.replyService.replyLead(
                    senderNumber,
                    lead.waitListNumber,
                );
                await this.replyService.replyLeadOption(sender);
            }
        }
    }
}